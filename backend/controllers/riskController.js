 ...
 * Returns: { totalAssets, highRiskCount, mediumRiskCount, lowRiskCount, averageRiskScore }
 */
exports.getRiskSummary = async (req, res) => {
	try {
		...
		const mediumRiskCount = allRisks.filter(
			(r) => r.riskLevel === "MEDIUM",
		).length;
		const lowRiskCount = allRisks.filter(
			(r) => r.riskLevel === "LOW",
		).length;
