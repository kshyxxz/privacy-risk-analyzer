...
exports.assignPiiToAsset = async (req, res) => {
	try {
		...
		for (const mapping of mappingsArray) {
			try {
				const risk = await calculateAssetRisk(mapping.asset_id);
				...
				console.log(
					`[assignPiiToAsset] Risk recalculated for asset ${mapping.asset_id}: ${risk.riskLevel} (${risk.riskScore}%)`,
				);
			} catch (riskError) {
				console.error(
					`Error recalculating risk for asset ${mapping.asset_id}:`,
					riskError,
				);
